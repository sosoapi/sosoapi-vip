...
public class ApacheMailServiceImpl implements MailService{
	...
	private void initEmail(Email email,String fromEmail, String fromPasswd,String fromName,
	    ...
	    email.setDebug(MailCfg.DEBUG);//是否开启调试默认不开启  
        email.setSSLOnConnect(MailCfg.SSL_ON_CONNECT);//开启SSL加密  
        email.setStartTLSEnabled(MailCfg.START_TLS_ENABLED);//开启TLS加密 
        email.setSmtpPort(Integer.parseInt(MailCfg.SMTP_PORT));
        email.setSslSmtpPort(MailCfg.SSL_SMTP_PORT);
