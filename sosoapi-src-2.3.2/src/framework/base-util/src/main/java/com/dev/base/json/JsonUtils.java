...
//			mapper.configure(SerializationFeature.WRITE_ENUMS_USING_INDEX,true);
	...
	/**
	 * 
			*@name 判断2个json字符串对应的json对象是否相等
			*@Description  
			*@CreateDate 2017年5月12日下午8:38:17
	 */
	public static boolean isEqual(String srcJson,String targetJson){
		if (StringUtils.isEmpty(srcJson) || StringUtils.isEmpty(targetJson)) {
			return false;
		}
		
		JsonParser srcParse = new JsonParser();  
        JsonParser targetParse = new JsonParser();  
        return ((JsonObject) srcParse.parse(srcJson)).equals((JsonObject) targetParse.parse(targetJson));
	}
