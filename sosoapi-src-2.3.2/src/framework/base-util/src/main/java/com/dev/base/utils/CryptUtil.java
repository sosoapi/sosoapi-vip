...
public class CryptUtil {
	 ...
	 * 
			*@name 生成安全密钥
			*@Description  
			*@CreateDate 2016年10月18日下午4:08:46
	 */
	public static byte[] buildSecretKey(String key){
		if (StringUtils.isEmpty(key) || !Pattern.matches("[A-Za-z0-9]{16}", key.trim())) {
			throw new RuntimeException("初始化安全密钥失败，密钥必须是由字母或数字组成的16位字符串。");
		}
		
		int length = 16;
		byte[] array = new byte[length];
		for (int i = 0; i < length; i++) {
			array[i] = (byte)key.charAt(i);
		}
