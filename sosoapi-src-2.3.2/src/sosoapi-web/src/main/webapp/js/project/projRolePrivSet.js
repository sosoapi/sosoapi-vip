...
function initPrivTree(){
	var setting = {
		async: {
			enable: true,
			url:"auth/proj/role/json/listTree.htm",
			otherParam:{"roleId":parseInt($("#roleId").val()),"projId":parseInt($("#projId").val())},
...
function initSubmit(){
	$("#saveRolePrivBtn").click(function(){
		...
		doPost("auth/proj/role/json/updatePriv.htm",param,function(interInfo){
			notice("更新成功",function(){
				redirectUrl("auth/proj/role/list.htm?projId=" + $("#projId").val() + "&docId=" + $("#docId").val());
			});
		});
	});
}
