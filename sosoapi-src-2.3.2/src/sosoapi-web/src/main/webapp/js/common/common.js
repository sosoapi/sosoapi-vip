...
function formatTimestamp(timestamp) {  
    ...
    return year + '-' + buildDateFormat(month)
    			+ '-' + buildDateFormat(date)
    			+ ' ' + buildDateFormat(hours)
    			+ ':' + buildDateFormat(minutes)
    			+ ':' + buildDateFormat(seconds);  
} 

function buildDateFormat(val){
...
function dealWithException(exception) {
	if(exception.errorCode == '50011'){//session过期，重新登录
		notice(exception.errorMsg,function(){
			redirectUrl("forwardLogin.htm");
		});
	}
	else {
...
//跳转到指定页面
function redirectUrl(url,isRelative){
