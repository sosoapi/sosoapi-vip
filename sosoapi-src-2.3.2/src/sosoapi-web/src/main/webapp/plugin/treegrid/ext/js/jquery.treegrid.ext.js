...
(function($) {
    ...
    function initTree(jqGridObjArray,options) {
        ...
        return jqGridObjArray.each(function() {
            ...
            //创建行号
            $this.treegrid('createNodeNumber');
        });
    }
    ...
    //获取缩进位置
    function getIndentCell(jqNodeObj){
        return jqNodeObj.find('td').get(jqNodeObj.treegrid('getSetting', 'indentColumn'));
    }
    
    //获取操作位置
    function getOperCell(jqNodeObj){
        return jqNodeObj.find('td').get(jqNodeObj.treegrid('getSetting', 'operColumn'));
    }
...
//            $(tpl).insertBefore(expander);
     ...
     * 初始化扩展信息
     */
    function initNodeExt(nodeObj){
    	//初始化行号
    	initNodeNumber(nodeObj);
    	
    	//初始化拖拽
    	initNodeDrag(nodeObj);
    	
    	//初始化新增子节点
    	initNodeAddChild(nodeObj);
    	
    	//初始化删除
    	initNodeRemove(nodeObj);
    }
    
    /**
     * 初始化行号
     */
    function initNodeNumber(nodeObj) {
        var $this = $(nodeObj);
        var expander = $this.find('.treegrid-expander');
        var tpl = $this.treegrid('getSetting', 'nodeNumberTemplate');
        
        var nodeNumber = $this.treegrid('getSetting', 'getNodeNumber').apply(this);
        if(nodeNumber){
        	nodeNumber.remove();
        }
     ...
     * 初始化新增节点
     */
    function initNodeAddChild(nodeObj){
    	var $this = $(nodeObj);
    	var cell = getOperCell($this);
        var tpl = $this.treegrid('getSetting', 'nodeAddChildTemplate');
        if(tpl){
        	$(tpl).prependTo(cell).click(function(){
        		var nodeTmpl = $this.treegrid('getSetting', 'getNodeTmpl').apply(this);
        		$(this).closest('tr').treegrid("addChildNode",$(nodeTmpl));
        	});
        }
     ...
     * 初始化删除节点
     */
    function initNodeRemove(nodeObj){
    	var $this = $(nodeObj);
    	var cell = getOperCell($this);
        var tpl = $this.treegrid('getSetting', 'nodeRemoveTemplate');
        if(tpl){
        	$(tpl).prependTo(cell).click(function(){
        		$this.treegrid("removeNodes");
        	});
        }
     ...
     * 初始化拖拽节点
     */
    function initNodeDrag(nodeObj){
    	var $this = $(nodeObj);
    	var cell = getOperCell($this);
        var tpl = $this.treegrid('getSetting', 'nodeDragTemplate');
        if(tpl){
        	$(tpl).prependTo(cell);
        }
   ...
   var methods = {
         ...
         * 创建序号
         */
        createNodeNumber: function(){
        	var $this = $(this);
        	if($this.treegrid('getSetting','showNodeNumber')){
        		$this.treegrid('getTreeContainer').find(".treegrid-node-number").each(function(index,element){
        ...
        removeNodes: function(){
        	...
        	if(parentNode){
        		//重新计算行号
            	parentNode.treegrid('createNodeNumber');
        		
            	//父节点重新渲染
            	parentNode.treegrid('render');
        	}
        	else{
        		tree.treegrid('createNodeNumber');
        	}
        },
    ...
    $.fn.treegrid.defaults = {
        ...
        nodeNumberTemplate: '<span class="treegrid-node-number"></span>',
        nodeRemoveTemplate: '<span class="treegrid-node-remove"></span>',
        nodeAddChildTemplate: '<span class="treegrid-node-add-child"></span>',
        nodeDragTemplate: '<span class="treegrid-node-drag"></span>',
        ...
        getNodeNumber: function() {
            return $(this).find('.treegrid-node-number');
        },
