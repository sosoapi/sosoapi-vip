...
<div class="sidebar-left sticky-sidebar">
    ...
    <div class="sidebar-left-info">
        ...
        <ul class="nav nav-pills nav-stacked side-navigation">
	        <c:if test="${not empty (appProjRolePrivMap[(userInfo.projRoleMap)[param.projId]]).privList}">
	        	<c:forEach items="${(appProjRolePrivMap[(userInfo.projRoleMap)[param.projId]]).privList}" var="privInfo">
	        			...
	        			<c:choose>
					                ...
					                <ul class="child-list">
					                	<c:forEach items="${privInfo.childList}" var="childInfo">
					                		<li class="${reqPrivInfo.menuCode == childInfo.code ? 'active' : ''}">
									            <a href="${childInfo.url}?projId=${param.projId}&docId=${param.docId}">
	        				...
	        				<c:otherwise>
	        					...
	        					<li class="${reqPrivInfo.menuCode == privInfo.code ? 'active' : ''}">
						            <a href="${privInfo.url}?projId=${param.projId}&docId=${param.docId}">
						            	<i class="${privInfo.iconClass}"></i> 
						            	<span>${privInfo.name}</span>
						            	<c:if test="${not empty privInfo.imgUrl}">
											<img alt="" src="${privInfo.imgUrl}">
