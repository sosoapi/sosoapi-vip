...
public class GraphDFSUtil {
	 ...
	 */
	public static List<Long> topoForLong(List<Long> vertexList,Map<Long, Set<Long>> vertexRelation){
	 ...
	 * 
			...
			*@CreateDate 2015年11月16日下午1:52:38
	 */
	public static List<String> topoForStr(List<String> vertexList,Map<String, Set<String>> vertexRelation){
		...
		int vertexCount = vertexList.size();
		int index = 0;
		Map<String, Integer> vertexIndex = MapUtils.newMap();
		Graph graph = new Graph(vertexCount);
		//添加顶点
		...
		for (String vertex : vertexList) {
			...
			for (String temp : tempList) {
				if (vertexIndex.containsKey(temp)) {
					graph.addEdge(vertexIndex.get(temp), currentIndex);
				}
			}
		}
