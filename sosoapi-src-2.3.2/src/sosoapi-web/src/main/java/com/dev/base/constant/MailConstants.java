...
public class MailConstants {
	 ...
	 * 监控异常邮件标题
	 */
	public final static String SUB_EMAIL_MONITOR_ALARM = "http监控-异常通知";
	
	/**
	 * 监控异常邮箱邮件模板
	 */
	public final static String TMPL_EMAIL_MONITOR_ALARM = loadTmpl("mail-tmpl/monitor-alarm.ftl");
	
	/**
	 * 监控恢复邮件标题
	 */
	public final static String SUB_EMAIL_MONITOR_RECOVER = "http监控-恢复通知";
	
	/**
	 * 监控恢复邮箱邮件模板
	 */
	public final static String TMPL_EMAIL_MONITOR_RECOVER = loadTmpl("mail-tmpl/monitor-recover.ftl");
	
	//加载模板内容
	private static String loadTmpl(String fileName){
		Resource fileRource = new ClassPathResource(fileName);
		try {
			return IOUtils.toString(fileRource.getInputStream(),AppConstants.DEF_CHARSET);
		} 
		catch (IOException e) {
			e.printStackTrace();
		}
