...
/**
 * 
		* <p>Title: 查看分享的 swagger api文档视图</p>
		* <p>Description: 描述（简要描述类的职责、实现方式、使用注意事项等）</p>
		* <p>CreateDate: 2015年8月13日下午2:20:08</p>
 */
@Controller
@RequestMapping("/pass/doc/view/share")
public class DocViewShareController extends BaseController{
	...
	public String forward(HttpServletRequest request,String shareKey){
		...
		DocView shareInfo = docViewService.decryptShareInfo(shareKey);
		ValidateUtils.notNull(shareInfo, ErrorCode.SYS_002,"非法的分享码");
		DocView docView = docViewService.getByDocId(shareInfo.getId(), shareInfo.getDocId());
		ValidateUtils.notNull(docView, ErrorCode.SYS_007,"文档视图不存在");
		ValidateUtils.isTrue(docView.isShare(), ErrorCode.SYS_006,"该文档视图已关闭分享");
		
		setShareInfo(request, shareKey, docView);
		
		String redirectUrl = WebUtil.getRedirectUrl("/pass/doc/view/share/show.htm?shareKey=" + shareKey);
		if (!StringUtils.isEmpty(docView.getSharePassword())) {//需要密码验证
			redirectUrl = WebUtil.getRedirectUrl("/pass/doc/view/share/forwardAuth.htm?shareKey=" + shareKey);
		}
	...
	public String forwardAuth(HttpServletRequest request,String shareKey,Model model){
		...
		String title = shareInfo.getDocView().getTitle();
		model.addAttribute("title", title);
		model.addAttribute("shareKey",shareKey);
		model.addAttribute("formAction","pass/doc/view/share/auth.htm");
	...
	public String auth(HttpServletRequest request,String shareKey,String password,Model model){
		...
		DocView docView = shareInfo.getDocView();
		if (docView.getSharePassword().equals(password)) {
			shareInfo.setAuth(true);
			updateShareInfo(request, shareInfo);
			return WebUtil.getRedirectUrl("/pass/doc/view/share/show.htm?shareKey=" + shareKey);
		}
		else{
			model.addAttribute("title", docView.getTitle());
			model.addAttribute("shareKey",shareKey);
	...
	public String show(HttpServletRequest request,String shareKey,Model model){
		...
		//直接访问，重定向到分享页面入口
		if (shareInfo == null) {
			return WebUtil.getRedirectUrl("/pass/doc/view/share/forward.htm?shareKey=" + shareKey);
		}
				
		String docUrl = CfgConstants.WEB_BASE_URL + "pass/doc/view/share/json/build.htm?shareKey=" + shareKey;
	...
	public @ResponseBody Swagger build(HttpServletRequest request,String shareKey,String condition){
		...
		DocView docView = shareInfo.getDocView();
		return swaggerJsonDocExportService.exportDocView(null, docView.getId(), docView.getDocId(), null, condition);
	}
	
	//获取分享信息
	private DocShareInfo getShareInfo(HttpServletRequest request,String shareKey){
		Map<String, DocShareInfo> shareInfoMap = (Map<String, DocShareInfo>)WebUtil.getSessionAttr(request, AppConstants.SESSION_KEY_SHARE_VIEW_INFO_MAP);
		return shareInfoMap == null ? null : shareInfoMap.get(shareKey);
	}
	
	//设置分享信息
	private void setShareInfo(HttpServletRequest request,String shareKey,DocView docView){
		Map<String, DocShareInfo> shareInfoMap = (Map<String, DocShareInfo>)WebUtil.getSessionAttr(request, AppConstants.SESSION_KEY_SHARE_VIEW_INFO_MAP);
		...
		//有设置密码，则需要授权
		shareInfo.setAuth(StringUtils.isEmpty(docView.getSharePassword()));
		shareInfo.setDocView(docView);
		shareInfo.setKey(shareKey);
		shareInfoMap.put(shareKey, shareInfo);
		
		WebUtil.setSessionAttr(request, AppConstants.SESSION_KEY_SHARE_VIEW_INFO_MAP, shareInfoMap);
	}
