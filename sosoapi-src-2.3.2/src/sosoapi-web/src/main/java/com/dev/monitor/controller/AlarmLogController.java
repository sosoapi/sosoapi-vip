 ...
 */
@Controller
@RequestMapping("/auth/monitor/alarm/log")
public class AlarmLogController extends BaseController{
	...
	public String list(HttpServletRequest request,Model model,String name,
		...
		List<AlarmLogInfo> list = alarmLogService.listByUserId(userId, name, sub, 
									monitorStatus, start, end,type, pager);
		int count = alarmLogService.countByUserId(userId, name, sub, 
	...
	public String getInfo(HttpServletRequest request,Model model,Long logId){
		ValidateUtils.notNull(logId, ErrorCode.SYS_001,"监控警报id不能为空");
		
		AlarmLogInfo logInfo = alarmLogService.getByUserId(getUserId(request), logId);
		model.addAttribute("logInfo", logInfo);
		
		return "monitor/alarmLog";
	}
}
