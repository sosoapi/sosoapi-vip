...
public class MailUtil {
	 ...
	 * 监控发送者邮箱
	 */
	public final static String MONITOR_FROM_EMAIL = mailCfg.getProperty("monitor.from.email");
	
	/**
	 * 监控发送者密码
	 */
	public final static String MONITOR_FROM_PASSWD = mailCfg.getProperty("monitor.from.passwd");
	
	/**
	 * 监控发送者名称
	 */
	public final static String MONITOR_FROM_NAME = mailCfg.getProperty("monitor.from.name");
	...
	/**
	 * 
			*@name 发送监控通知，使用monitor账号
			*@Description  
			*@CreateDate 2015年10月24日下午4:33:09
	 */
	public static void sendMonitor(final List<String> emailList,String subject,String content) {
		if (CollectionUtils.isEmpty(emailList)) {
			return ;
		}
		...
		TaskUtils.execAsyn(new Runnable() {
			@Override
			public void run() {
				mailService.sendMail(MONITOR_FROM_EMAIL, MONITOR_FROM_PASSWD, MONITOR_FROM_NAME, 
										MailCfg.HOST, emailList, mailMsg);
			}
		});
	}
