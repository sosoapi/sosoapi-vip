...
public class BaseExceptionHandler implements HandlerExceptionResolver {
	...
	public ModelAndView resolveException(HttpServletRequest request, HttpServletResponse response, 
		...
		//判断是否是ajax请求
		boolean isAjaxReq = WebUtil.isAjaxReq(request) || isMockReq(request);
		if (!isAjaxReq && request.getMethod().equals("GET")) {
			String redirectUrl = request.getParameter("redirectUrl");
			if (StringUtils.isEmpty(redirectUrl)) {
				redirectUrl = WebUtil.getRequestURI(request) + "?" + WebUtil.getParamStr(request);
			}
			WebUtil.setSessionAttr(request, AppConstants.SESSION_KEY_REDIRECT_URL, redirectUrl);
		}
	...
	//判断是否是mock请求
	private boolean isMockReq(HttpServletRequest request){
		return request.getRequestURI().contains("/pass/mock/");
	}
	...
	private String parseViewName(Exception ex,boolean isAjaxReq){
		...
		if (ex instanceof SessionTimeoutException) {//登陆超时异常
			viewName = "redirect:/forwardLogin.htm";
		}
		else if (ex instanceof AuthException || ex instanceof AuthorizationException) {//无权限异常
