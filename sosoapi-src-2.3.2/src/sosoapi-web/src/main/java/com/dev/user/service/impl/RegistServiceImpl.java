...
public class RegistServiceImpl implements RegistService{
	...
	public void registByEmail(RegistParamInfo registParamInfo) {
		...
		if (CfgConstants.REGIST_EMAIL_VALID) {//需验证邮箱
			//发送邮件
	...
	private void sendEmail(String nickName,String registEmail){
		String code = CryptUtil.encryptAES(registEmail,CfgConstants.SECURITY_SECRET_KEY);
		
		Map<String,Object> model = MapUtils.newMap();
		model.put("nickName", nickName);
		model.put("activeUrl", CfgConstants.WEB_BASE_URL + "regist/active.htm?code=" + code);
		
		MailUtil.send(registEmail, MailConstants.SUB_REGIST + CfgConstants.WEB_NAME, MailConstants.TMPL_REGIST, model);
	}
	...
	private Long saveUserInfo(RegistParamInfo registParamInfo){
		...
		userBasic.setValid(!CfgConstants.REGIST_EMAIL_VALID);
		userBasic.setRoleId(CfgConstants.ROLE_ID_DEF);
	...
	public void activeByEmail(String code) {
		String registEmail = CryptUtil.decryptAES(code,CfgConstants.SECURITY_SECRET_KEY);
