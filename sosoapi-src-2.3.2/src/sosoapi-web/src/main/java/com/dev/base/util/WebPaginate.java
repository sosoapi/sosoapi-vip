...
public class WebPaginate {
    ...
    public static WebPaginate build(HttpServletRequest request,Integer pageNumber,
    	...
    	return build(request, pageNumber, pageSize, totalCount, CfgConstants.SHOW_PAGE_NUMBER);
    }
     ...
     */
    public static WebPaginate build(HttpServletRequest request,Integer pageNumber,
									Integer pageSize,int totalCount,int showPageNumber){
    	if (totalCount == 0) {
			return null;
		}
    	...
    	paginate.setPageStart(paginate.calculatePageStart(pageNumber,totalPage,showPageNumber,pageEnd));
    ...
    private String buildUrl(HttpServletRequest request,int pageSize){
    	...
    	urlBuilder.append(WebUtil.getRequestURI(request))
    			  .append("?");
    ...
    //计算当前分页第一个页码
    private int calculatePageStart(int pageNumber,int totalPage,int showPageNumber,int pageEnd){
    	int middle = showPageNumber / 2;
    	if ((totalPage < showPageNumber) || (pageNumber <= (middle + 1))) {//总页数小于展示页码数或当前页小于中间值
			return 1;
		}
    	else{
    		return pageEnd - showPageNumber + 1;
    	}
	}
	
    //计算当前分页最后一个页码
    private int calculatePageEnd(int pageNumber,int totalPage,int showPageNumber){
    	if (totalPage < showPageNumber) {//总页数小于展示页码数
			return totalPage;
		}
    	...
    	int pageEnd = 0;
    	if (showPageNumber % 2 == 0) {//处理展示页为偶数情况
